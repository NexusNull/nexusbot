...
export default class StreamNotifyModule {
    ...
    async init() {
        ...
        for (let sub of subs) {
            let res = await this.twitcheventsub.addSubscription(sub.broadcasterId)
            if(!res)
    ...
    async addChannelModalInteraction(interaction) {
        ...
        let result = await this.twitcheventsub.addSubscription(channelId); //TOD check if this causes any errors. i.e. http
        if(!result){
            return interaction.reply({content: "Error: The channel cloud not be watched! Please check the id.", flags: MessageFlags.Ephemeral})
        }
    ...
    async removeChannelModalInteraction(interaction) {
        ...
        if (sub.addedBy === interaction.user.id) {
            interaction.reply({content: "Error: The channel is already being watched!", flags: MessageFlags.Ephemeral})
        }

        await this.twitcheventsub.addSubscription(channelId); //TOD check if this causes any errors. i.e. http
        await this.subscriptions.createSubscription(channelId, interaction)
